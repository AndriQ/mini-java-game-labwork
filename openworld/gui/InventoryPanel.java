...
public class InventoryPanel extends JPanel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == consumeButton && !consumeList.isSelectionEmpty()) {
            int value = consumeList.getSelectedIndex();
            inventory.useItem(value);
            consumeList.clearSelection();
        }
        for (InventorySlotType type : InventorySlotType.values()) {
            if (e.getSource() == equipButtons.get(type)) {
                if (!equipLists.get(type).isSelectionEmpty()) {
                    ...
                    if (selectedIndex >= 0) {
                        ...
                        if (itemToEquip != null) {
                            ...
                            equipLists.remove(type, itemToEquip);
                            equipLists.get(type).clearSelection();
                        }
                    }
                }
            }
